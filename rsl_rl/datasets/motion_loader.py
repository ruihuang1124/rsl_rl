...
class AMPLoader:
    ...
    def __init__(
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            with open(motion_file, "r") as f:
                ...
                motion_data = self.reorder_from_isaacgym_to_isaacsim(motion_data)
    ...
    def reorder_from_isaacgym_to_isaacsim(self, motion_data):
        """Convert motion data from Isaac Gym (depth-first) to Isaac Sim (breadth-first) ordering.
        
        Rearranges joint order from Isaac Gym [leg1_j1, leg1_j2, leg1_j3, leg2_j1, ...]
        to Isaac Sim order [leg1_j1, leg2_j1, leg3_j1, leg4_j1, leg1_j2, leg2_j2, ...].
        
        Applies to: joint positions, foot positions, joint velocities, foot velocities.
        Preserves: root position, root rotation, linear velocity, angular velocity.
        
        Args:
            motion_data: numpy array of shape (num_frames, 61) in Isaac Gym ordering
            
        Returns:
            numpy array of shape (num_frames, 61) in Isaac Sim ordering
        """
        root_pos = AMPLoader.get_root_pos_batch(motion_data)
        root_rot = AMPLoader.get_root_rot_batch(motion_data)
        
        # Reorder joint positions: Isaac Gym [4 legs x 3 joints] -> Isaac Sim [3 joints x 4 legs]
        joint_pos = AMPLoader.get_joint_pose_batch(motion_data)
        joint_pos_reshaped = np.reshape(joint_pos, (-1, 4, 3))
        joint_pos_reordered = np.transpose(joint_pos_reshaped, (0, 2, 1))
        joint_pos_reordered = np.reshape(joint_pos_reordered, (-1, 12))
        
        # Reorder foot positions: Isaac Gym [4 legs x 3 coords] -> Isaac Sim [3 coords x 4 legs]
        foot_pos = AMPLoader.get_tar_toe_pos_local_batch(motion_data)
        foot_pos_reshaped = np.reshape(foot_pos, (-1, 4, 3))
        foot_pos_reordered = np.transpose(foot_pos_reshaped, (0, 2, 1))
        foot_pos_reordered = np.reshape(foot_pos_reordered, (-1, 12))
        
        lin_vel = AMPLoader.get_linear_vel_batch(motion_data)
        ang_vel = AMPLoader.get_angular_vel_batch(motion_data)
        
        # Reorder joint velocities: Isaac Gym [4 legs x 3 joints] -> Isaac Sim [3 joints x 4 legs]
        joint_vel = AMPLoader.get_joint_vel_batch(motion_data)
        joint_vel_reshaped = np.reshape(joint_vel, (-1, 4, 3))
        joint_vel_reordered = np.transpose(joint_vel_reshaped, (0, 2, 1))
        joint_vel_reordered = np.reshape(joint_vel_reordered, (-1, 12))
        
        # Reorder foot velocities: Isaac Gym [4 legs x 3 coords] -> Isaac Sim [3 coords x 4 legs]
        foot_vel = AMPLoader.get_tar_toe_vel_local_batch(motion_data)
        foot_vel_reshaped = np.reshape(foot_vel, (-1, 4, 3))
        foot_vel_reordered = np.transpose(foot_vel_reshaped, (0, 2, 1))
        foot_vel_reordered = np.reshape(foot_vel_reordered, (-1, 12))
        
        return np.hstack(
            [root_pos, root_rot, joint_pos_reordered, foot_pos_reordered, 
             lin_vel, ang_vel, joint_vel_reordered, foot_vel_reordered])
